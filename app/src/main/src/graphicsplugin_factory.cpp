...
namespace
{
using GraphicsPluginFactory =
    std::function<std::shared_ptr<IGraphicsPlugin>(const std::shared_ptr<Options>& options, std::shared_ptr<IPlatformPlugin> platformPlugin)>;
...
#endif
#ifdef XR_USE_GRAPHICS_API_OPENGL
    {"OpenGL", [](const std::shared_ptr<Options>& options, std::shared_ptr<IPlatformPlugin> platformPlugin)
...
#endif
#if defined(XR_USE_GRAPHICS_API_D3D11) && !defined(MISSING_DIRECTX_COLORS)
    {"D3D11", [](const std::shared_ptr<Options>& options, std::shared_ptr<IPlatformPlugin> platformPlugin)
